...
bool MediaCrypto::SetKey(int type, int cs, const uint8_t* key, size_t len) {
  if (session_) {
    RTC_LOG(LS_ERROR) << "Failed to create MediaCrypto SRTP session: "
  ...
  if (cs == rtc::SRTP_AES128_CM_SHA1_80) {
    ...
    RTC_LOG(LS_WARNING) << "Failed to create SRTP session: unsupported"
                    << " cipher_suite " << cs;
    return false;
  }
  ...
  if (!rtc::GetSrtpKeyAndSaltLengths(cs, &expected_key_len,
    ...
    RTC_LOG(LS_WARNING) << "Failed to create MediaCrypto SRTP session: unsupported"
  ...
  if (!key ||
    ...
    RTC_LOG(LS_WARNING) << "Failed to create MediaCrypto SRTP session: invalid key";
    return false;
  }
  ...
  if (err != srtp_err_status_ok) {
    session_ = nullptr;
    RTC_LOG(LS_ERROR) << "Failed to create MediaCrypto SRTP session, err=" << err;
    return false;
  }
...
bool MediaCrypto::ProtectRtp(void* p, int in_len, int max_len, int* out_len) {
  if (!session_) {
    RTC_LOG(LS_WARNING) << "Failed to protect MediaCrypto SRTP packet:"
                    << " no SRTP Session";
    return false;
  }
  ...
  *out_len = in_len;
  ...
  if (err != srtp_err_status_ok) {
    RTC_LOG(LS_WARNING) << "Failed to end to end encrypt media packet";
    return false;
  }
  return true;
}
...
bool MediaCrypto::UnprotectRtp(void* p, int in_len, int* out_len) {
  if (!session_) {
    RTC_LOG(LS_WARNING) << "Failed to unprotect MediaCrypto SRTP packet:"
                    << " no SRTP Session";
    return false;
  }
  ...
  *out_len = in_len;
  ...
  if (err == srtp_err_status_replay_fail) {
    RTC_LOG(LS_WARNING) << "Replay failed";
...
bool MediaCrypto::Encrypt(RtpPacket* packet) {
  if (!session_) {
    RTC_LOG(LS_WARNING) << "Failed to encrypt MediaCrypto RTP packet:"
                    << " no SRTP Session";
    return false;
  }
  ...
  if (encrypted_payload_size > packet->MaxPayloadSize()) {
    RTC_LOG(LS_WARNING) << "Failed to perform End to End media encryption"
...
bool MediaCrypto::Decrypt(uint8_t* payload, size_t* payload_length) {
  if (!session_) {
    RTC_LOG(LS_WARNING) << "Failed to decrypt MediaCrypto RTP packet:"
                    << " no SRTP Session";
    return false;
  }
